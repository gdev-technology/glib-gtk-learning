...
The GLib libraries can be used to write operating system services (daemons, …), libraries, command line utilities and such like. GLib offers higher-level APIs than the POSIX standard, it is therefore more comfortable to write a C program with GLib.
...
There are new GLib, GTK+ and GNOME stable releases every six months, around March and September. A version number has the form X.Y.Z. ``Y'' is even for stable versions, and is odd for unstable versions. For example, the 3.14 version is stable, but 3.15 is unstable. A new minor stable version (e.g. 3.14.0 $\rightarrow$ 3.14.1) doesn't add new features, only translation updates, bug fixes and performance improvements. GNOME components are meant to be installed with the same versions, alongside the version of GTK+ and GLib released at the same time; it's a bad idea to run a GNOME application in version 3.14 with GTK+~3.12, for instance. Currently GNOME and GTK+ have the same version numbers, but not GLib (GLib~2.42 was released at the same time as GTK+~3.14). For a library, a new major version number (``X'' in X.Y.Z) generally means there has been an API break, but hopefully previous major versions are parallel-installable with the new version. During a development cycle (e.g 3.15), there is no API stability guarantees for \emph{new} features.

More information about GNOME: \url{http://www.gnome.org/}

\section{Prerequisites}

Here is a list of prerequisites to read this book.
